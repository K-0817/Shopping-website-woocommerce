        ...
        $order_key = $order->get_order_number();
 		if ( in_array( 'agent', $user_roles, true ) ) {
			...
			if(intval($user_id) === intval($agent_user)){
				...
				$order->update_meta_data( 'assign_location', $agent_location );
				//$order->update_meta_data( 'assign_location', $agent_location );
				//update_post_meta( $order_id, 'assign_location', $agent_location );
			}
			endforeach;
		}else{
			$agent_first_last_name = explode(' ', $agent_name);
			...
			$order->update_meta_data( 'sales_agent', $agent_name );
