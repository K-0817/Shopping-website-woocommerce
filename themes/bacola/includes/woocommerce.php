...
use Kapu\Core\Services\SavingsStatistics;
...
function bacola_cart_with_quantity($id){
	...
	$output .= '<input type="text" 
		...
		name="quantity" step="'.esc_attr($step_quantity).'" 
		min="'.esc_attr($min_quantity).'" 
		max="'.esc_attr($max_quantity).'" 
		value="'.esc_attr($in_cart_value).'" 
		title="Menge" 
		size="4" 
		inputmode="numeric">';
...
function bacola_product_type1(){
	...
	$tl_variations = $allproduct->get_available_variations();
	$base_variation_id = -1;
	$pamoja_variation_id = -1;
	$base_maxQnt = 0;
	$base_minQnt = 0;
	$pamoja_maxQnt = 0;
	$pamoja_minQnt = 0;
    $stock_qnt_base = $allproduct->get_stock_quantity();
    $stock_qnt_pamoja = $allproduct->get_stock_quantity();
	$now_in_cart_base = 0;
	$now_in_cart_pamoja = 0;
	$link_to_remove_base = '';
	$link_to_remove_pamoja = '';
	// dd($tl_variations[1]['min_qty']);
	// dd($tl_variations[0]['max_qty']);
	$base_price = 0;
	$pamoja_price = 0;
	foreach($tl_variations as $tl_var){
		$formatted_price = $tl_var['display_price'];
		$dot_pos = intval( strpos($formatted_price, "."));
		if($dot_pos == 0){
			$formatted_price .= ".00";
		} else if(strlen($formatted_price) - $dot_pos == 2){
			$formatted_price .= "0";
		}
		if($tl_var['attributes']['attribute_pa_pricing'] == 'base'){
			$base_variation_id = $tl_var['variation_id'];
			$_cart_item_key = tl_get_cart_item_key($id, $tl_var['variation_id']);
			$now_in_cart_base = WC()->cart->get_cart_item( $_cart_item_key )['quantity'] ?? 0;
			$base_price = $formatted_price;
			$base_maxQnt = $tl_var['max_qty'];
			$base_minQnt = $tl_var['min_qty'];
			$link_to_remove_base = wc_get_cart_remove_url( $_cart_item_key );
		}
		if($tl_var['attributes']['attribute_pa_pricing'] == 'pamoja'){ 
			$pamoja_variation_id = $tl_var['variation_id'];
			$_cart_item_key = tl_get_cart_item_key($id, $tl_var['variation_id']);
			$now_in_cart_pamoja = WC()->cart->get_cart_item( $_cart_item_key )['quantity'] ?? 0;
			$pamoja_maxQnt = $tl_var['max_qty'];
			$pamoja_minQnt = $tl_var['min_qty'];
			$pamoja_price = $formatted_price;
			$link_to_remove_pamoja = wc_get_cart_remove_url( $_cart_item_key );
		}
	}

	$savings_amount = SavingsStatistics::getProductSavings($allproduct, 1, true);
	...
	foreach(WC()->cart->cart_contents as $key => $value){
		// dd($value['variation_id']);
		// dd($value['product_id']);
		if($value['product_id'] == $id){
			// echo $value['quantity'] . "|";
			if($value == $base_variation_id)
				$in_cart_base = $value['quantity'];
			else if($value == $pamoja_variation_id)
				$in_cart_pamoja = $value['quantity'];
		}
	}
	// dd();
	// if($in_cart_base != 0 || $in_cart_pamoja != 0)
	// 	dd($in_cart_base, $in_cart_pamoja);
	// dd(WC()->cart->find_product_in_cart( $cart_id ));
	// if(  ) {
	...
	if(bacola_shop_view() == 'list_view' || $postview == 'list_view') {
		...
		// $output .= '</div>';
		// }
		// $output .= '<div class="product-info-block">';
		// $output .= '<span class="product-info-link"><img src="/wp-content/themes/bacola-mob-child/assets/img/info.png" alt="">How Pamoja works</span>';
		// $output .= '</div>';
		// $output .= '<div class="product-share-block">';
		// $output .= '<div class="product-share-title">Share & help others save!</div>';
		// $output .= '<div class="product-share-items-block">';
		// $output .= '<a href="whatsapp://send?text='.get_permalink().'" data-action="share/whatsapp/share" target="_blank"><img src="/wp-content/themes/bacola-mob-child/assets/img/watsapp.png" width="20" alt=""></a>';
		...
		$output = "";
		$output .= '<div class="catalog__item '.($stock_status != 'instock' ? 'catalog__item--out' : "").'" attr-js-prod-wrap>';
		$output .= '<div>';
		$output .= '<div>';

		$output .= '<div class="catalog__item-img img-wrap">';
		$output .= '<div>';
		$output .= '<picture>
						<img src="'.bacola_product_image().'" alt="'.the_title_attribute( 'echo=0' ).'">
					</picture>';
		$output .= '</div>';
		if($stock_status != 'instock')
			$output .= '<span>out of stock</span>';
		$output .= '</div>';
		$output .= '<div class="catalog__item-title">'.get_the_title().'</div>';
		$output .= '</div>';
		$output .= '<div>';

		$output .= '<div class="catalog__btn catalog__btn--base" 
			onclick="tl_addToCartWrap.handler_AddProduct(`base`, this)"
			data-in-cart="'.$now_in_cart_base.'"
			data-min-qnt="'.$base_minQnt.'"
			data-max-qnt="'.$base_maxQnt.'"
			data-stock-amount="'.$stock_qnt_base.'"
			data-prod-id="'.$id.'"
			data-variation-id="'.$base_variation_id.'"
			data-rm-link="'.$link_to_remove_base.'"
			>';
		$output .= '<div class="catalog__btn-title">Base - </div>';
		$output .= '<div class="catalog__btn-price">KSh '.$base_price.'</div>';
		$output .= '</div>';
		// onclick="">';
		$output .= '<div class="catalog__btn catalog__btn--pamoja"
			onclick="tl_addToCartWrap.handler_AddProduct(`pamoja`, this)"
			data-in-cart="'.$now_in_cart_pamoja.'"
			data-min-qnt="'.$pamoja_minQnt.'"
			data-max-qnt="'.$pamoja_maxQnt.'"
			data-stock-amount="'.$stock_qnt_pamoja.'"
			data-prod-id="'.$id.'"
			data-variation-id="'.$pamoja_variation_id.'"
			data-rm-link="'.$link_to_remove_pamoja.'"
			data-savings-per-item="'.$savings_amount.'"
		>';
		$output .= '<div class="catalog__btn-title">Price</div>';
		$output .= '<div class="catalog__btn-price">KSh '.$pamoja_price.'</div>';
		$output .= '</div>';
		$output .= '<div class="catalog_savings"><svg width="11px" height="11px" viewBox="0 0 11 11" >
    <g id="🚀-ORDERS" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <g id="Agent-Checkout---Order-Details/Status" transform="translate(-199.000000, -465.000000)">
            <g id="accounts-section" transform="translate(0.000000, 62.000000)">
                <g id="Group-9" transform="translate(0.000000, 318.000000)">
                    <g id="PDP-section" transform="translate(0.000000, 20.000000)">
                        <g id="basket-item-1" transform="translate(20.000000, 18.000000)">
                            <g id="Group-4-Copy" transform="translate(179.000000, 42.000000)">
                                <image id="Price-Tag" x="0" y="5" width="11" height="11" xlink:href="data:image/png;base64,"></image>
                            </g>
                        </g>
                    </g>
                </g>
...
</svg>Save KSh '. $savings_amount.'</div>';
		$output .= '</div>';

		// $output .= '<div class="dn" attr-js-hidden-form>';
		// $output .= bacola_cart_with_quantity($id);
		// $output .= '</div>';
		$output .= '</div>';
		$output .= '</div>';
