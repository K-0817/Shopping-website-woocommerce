...
require_once get_template_directory() . '/TimLis/TimCode.php';
...
function bacola_scripts() {
	...
	wp_enqueue_style( 'bacola-style',         	get_stylesheet_uri(), false, filemtime( get_theme_file_path('/style.css') ) );
...
function wpbl_save_fields( $order_id ){
    ...
    if( !empty( $_POST['sales_agent'] ) ){
        $agent_id = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_title = '" . sanitize_text_field( $_POST['sales_agent']) . "' AND post_type = 'agent' AND post_status = 'publish'" );
        update_post_meta( $order_id, 'sales_agent', $agent_id );
    }

    if( !empty( $_POST['pickup_locations'] ) ){
        update_post_meta( $order_id, 'shipping_address', sanitize_text_field( $_POST['pickup_locations'] ) );
    }
